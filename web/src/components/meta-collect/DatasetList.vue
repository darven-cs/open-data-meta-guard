...
<template>
  <div class="dataset-list">
    ...
    <!-- 分页 -->
    <div v-if="totalPages > 1" class="dataset-list__pager">
      <button
        type="button"
        class="dataset-list__page-btn"
        :disabled="page <= 1"
        @click="emit('page-change', page - 1)"
      >
        ← 上一页
      </button>
      <span class="dataset-list__page-info">
        {{ page }} / {{ totalPages }}
      </span>
      <button
        type="button"
...
.dataset-list__page-btn:hover:not(:disabled) { border-color: var(--accent); color: var(--accent); }
.dataset-list__page-btn:disabled { opacity: 0.4; cursor: not-allowed; }

.dataset-list__page-info {
  font-family: var(--font-mono);
  font-size: var(--text-xs);
  color: var(--ink-mute);
}
